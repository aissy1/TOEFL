...
class TestUnitController extends Controller
{
    public function ThrowSession(Request $request)
    {
        ...
        if ($account && !password_verify($password, $account->password)) {
            return redirect()->back()->withErrors(['account' => 'Invalid credentials.']);
        }
        ...
        session(['username' => $username, 'toefl_id' => $toefl->id]);
    ...
    private function getReadingQuestions($toeflSubtestId = 49, $subtest_id = 1)
    {
        $passage = Passage::where('toefl_subtest_id', $toeflSubtestId)
                    ...
                    'questions' => $items->map(function ($q) {
                        return [
                            ...
                            'correctAnswer' =>
                                ...
                                ? $q->choices[$q->correct_answer] ?? null
                                : null,
                        ];
                    })->values(),
                ];
